...
namespace CrazyMinnow.SALSA.Sync
{
    ...
    public class CM_SalsaSync : MonoBehaviour
    {
        ...
        private Vector3 eyeStartRot = Vector3.zero; // Stores the eye start rotation
        private Vector3 eyeNewRot = Vector3.zero; // Stores the eye new rotation
		...
		void Start()
        {
            ...
            if (leftEyeBone)
            {
                eyeStartRot = leftEyeBone.localEulerAngles;
            }
            if (rightEyeBone)
            {
                eyeStartRot = rightEyeBone.localEulerAngles;
            }
        }
            ...
            if (re3DEyes)
            {
                ...
                else if (eyeControl == AnimControl.Bones)
                {
                    ...
                    if (leftEyeBone || rightEyeBone)
                    {
                        ...
                        eyeNewRot = new Vector3(0f, 0f, 0f); // Final output: eyeStartRot + lookAmounts
                        ...
                        eyeNewRot = eyeStartRot + lookAmounts; // Add the combined amounts to the starting euler rotation

                        if (leftEyeBone) leftEyeBone.transform.localRotation = Quaternion.Euler(eyeNewRot);
                        if (rightEyeBone) rightEyeBone.transform.localRotation = Quaternion.Euler(eyeNewRot);
                    }
                }
            }
